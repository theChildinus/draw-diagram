...
直接生成或修改原生 Draw.io XML。完整读取 `xml.md` 后再写。
...
命令把目标基线和候选分别复制到目标同目录的私有暂存文件，对固定候选快照进行静态校验、可选渲染连线检查和所有请求的导出。候选存在警告时，同样检查固定目标基线，并按诊断 `fingerprint` 区分存量、新增和变差。所有错误、未接受的新增或变差警告、视觉门禁、页码及目标哈希检查全部通过后，才替换目标和导出物；正常进程错误会恢复已有文件。每个文件使用原子替换，多文件交付在捕获到错误时执行回滚，但不能把操作系统崩溃时的跨文件状态描述成数据库事务。

紧凑 JSON 回执固定包含：

- `status` 与 `committed`。
- 候选、目标修改前后及导出物的 SHA-256 和字节数。
- 校验页数、错误数、警告数、渲染连线检查及警告基线差分。
